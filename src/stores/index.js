/**
 * Desidia v2 - Store Index
 * 
 * Central export point for all Pinia stores
 */

export { useAuthStore } from './auth.store'
export { useWorkspaceStore } from './workspace.store'
export { useProjectStore } from './project.store'
export { useTaskStore } from './task.store'
export { useKanbanColumnStore } from './kanbanColumn.store'
export { useUIStore } from './ui.store'
export { useNotificationStore } from './notification.store'
export { useInboxStore } from './inbox.store'
export { useAIStore } from './ai.store'
export { useAIChatStore } from './aiChat.store'
export { useUserStore } from './user.store'
export { useRoleStore } from './role.store'
export { useGroupStore } from './group.store'
export { useBulletinStore } from './bulletin.store'
export { useCategoryStore } from './category.store'
export { useProjectPropertyStore } from './projectProperty.store'
