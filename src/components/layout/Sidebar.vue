...
<template>
  ...
  >
    ...
    <aside
      ref="sidebarRef"
      class="sidebar relative h-full border-r border-gray-200 bg-[#faf9fb] will-change-[width] overflow-x-hidden"
      :class="{ 'transition-[width] duration-150': !isResizing }"
      :style="{ width: uiStore.isMobile ? '288px' : `${uiStore.sidebarCustomWidth}px` }"
    >
      <div class="absolute inset-0 sidebar-scroll">
        <div class="sticky top-0 z-10 bg-[#faf9fb] pt-4 pb-3 px-3 border-b border-transparent">
          <!-- Header -->
          ...
          <button
            ...
            class="flex items-center gap-2 w-full py-2.5 px-3 bg-transparent border border-dashed border-gray-300 rounded-lg text-gray-500 text-sm cursor-pointer mb-2 hover:bg-gray-100 hover:border-gray-400 transition-all"
          >
        ...
        <div class="pb-10 pt-4 space-y-4">
          ...
          <nav class="mb-6">
            <button
              ...
              :class="[
                'flex items-center gap-2.5 py-2 px-3 rounded-lg text-sm cursor-pointer transition-all w-full min-w-0 group relative pr-8',
                item.active
            ...
            >
              ...
              <span class="menu-ellipsis absolute right-2 top-1/2 -translate-y-1/2 text-xs font-semibold text-gray-400 pointer-events-none">⋮</span>
              <span v-if="item.badge" class="bg-violet-500 text-white text-[11px] py-0.5 px-2 rounded-full font-medium ml-1">
                {{ item.badge }}
              </span>
            </button>
            ...
            <div class="dropdown-section mb-1 relative">
              <button
                ...
                class="group flex items-center gap-2.5 py-2 px-3 rounded-lg text-gray-600 text-sm cursor-pointer hover:bg-gray-100 transition-all w-full relative pr-8"
              >
          ...
          <div v-for="space in spaces" :key="space.id" class="dropdown-section mb-1 relative">
            <button
              ...
              class="group flex items-center gap-2.5 py-2 px-3 rounded-lg text-gray-600 text-sm cursor-pointer hover:bg-gray-100 transition-all w-full min-w-0 relative pr-8"
            >
              <span class="w-5 h-5 relative flex items-center justify-center">
            ...
            >
              <button
                ...
                class="submenu-item group flex items-center gap-2.5 py-1.5 px-3 pl-10 rounded-md text-gray-500 text-[13px] cursor-pointer hover:bg-gray-100 hover:text-gray-900 transition-all w-full text-left min-w-0 relative pr-8"
              >
                <span :class="['w-[18px] h-[18px] rounded flex items-center justify-center text-[10px]', item.color]">{{ item.emoji }}</span>
                <span class="flex-1 truncate">{{ item.name }}</span>
                <span class="child-menu-ellipsis absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-semibold text-gray-400 pointer-events-none">⋮</span>
              </button>
            </div>
          </div>
        </div>
